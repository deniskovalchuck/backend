...
namespace Core\PhpSpreadsheet\Writer\Xls;

use Core\PhpSpreadsheet\Cell\Coordinate;
use Core\PhpSpreadsheet\Cell\DataType;
use Core\PhpSpreadsheet\Cell\DataValidation;
use Core\PhpSpreadsheet\Exception as PhpSpreadsheetException;
use Core\PhpSpreadsheet\RichText\RichText;
use Core\PhpSpreadsheet\RichText\Run;
use Core\PhpSpreadsheet\Shared\StringHelper;
use Core\PhpSpreadsheet\Shared\Xls;
use Core\PhpSpreadsheet\Style\Alignment;
use Core\PhpSpreadsheet\Style\Border;
use Core\PhpSpreadsheet\Style\Color;
use Core\PhpSpreadsheet\Style\Conditional;
use Core\PhpSpreadsheet\Style\Fill;
use Core\PhpSpreadsheet\Style\Protection;
use Core\PhpSpreadsheet\Worksheet\PageSetup;
use Core\PhpSpreadsheet\Worksheet\SheetView;
...
class Worksheet extends BIFFwriter
{
     ...
     * Formula parser.
     *
     * @var \Core\PhpSpreadsheet\Writer\Xls\Parser
     */
    private $parser;
     ...
     * Sheet object.
     *
     * @var \Core\PhpSpreadsheet\Worksheet\Worksheet
     */
    public $phpSheet;
     ...
     *
     * @var \Core\PhpSpreadsheet\Shared\Escher
     */
    private $escher;
     ...
     * @param \Core\PhpSpreadsheet\Worksheet\Worksheet $phpSheet The worksheet to write
     */
    public function __construct(&$str_total, &$str_unique, &$str_table, &$colors, Parser $parser, $preCalculateFormulas, \Core\PhpSpreadsheet\Worksheet\Worksheet $phpSheet)
    {
...
//        if ($this->firstColumnIndex > 255) $this->firstColumnIndex = 255;
     ...
     *
     * @see \Core\PhpSpreadsheet\Writer\Xls\Workbook::storeWorkbook()
     */
    public function close(): void
    {
        ...
        if (($defaultWidth = $phpSheet->getDefaultColumnDimension()->getWidth()) < 0) {
            $defaultWidth = \Core\PhpSpreadsheet\Shared\Font::getDefaultColumnWidthByFont($phpSheet->getParent()->getDefaultStyle()->getFont());
        }
    ...
    private function writeBreaks(): void
    {
        ...
        foreach ($this->phpSheet->getBreaks() as $cell => $breakType) {
            ...
            switch ($breakType) {
                case \Core\PhpSpreadsheet\Worksheet\Worksheet::BREAK_COLUMN:
                    // Add to list of vertical breaks
                    $vbreaks[] = Coordinate::columnIndexFromString($coordinates[0]) - 1;

                    break;
                case \Core\PhpSpreadsheet\Worksheet\Worksheet::BREAK_ROW:
                    // Add to list of horizontal breaks
                    $hbreaks[] = $coordinates[1];

                    break;
                case \Core\PhpSpreadsheet\Worksheet\Worksheet::BREAK_NONE:
                default:
                    // Nothing to do
                    break;
            }
     ...
     * Get Escher object.
     *
     * @return \Core\PhpSpreadsheet\Shared\Escher
     */
     ...
     * Set Escher object.
     *
     * @param \Core\PhpSpreadsheet\Shared\Escher $pValue
     */
    public function setEscher(?\Core\PhpSpreadsheet\Shared\Escher $pValue = null): void
    {
        $this->escher = $pValue;
    }
    ...
    private function writeCFRule(Conditional $conditional): void
    {
        ...
        if ($bFormatFont == 1) {
            ...
            switch ($conditional->getStyle()->getFont()->getUnderline()) {
                case \Core\PhpSpreadsheet\Style\Font::UNDERLINE_NONE:
                    $dataBlockFont .= pack('C', 0x00);
                    $fontUnderline = 0;

                    break;
                case \Core\PhpSpreadsheet\Style\Font::UNDERLINE_DOUBLE:
                    $dataBlockFont .= pack('C', 0x02);
                    $fontUnderline = 0;

                    break;
                case \Core\PhpSpreadsheet\Style\Font::UNDERLINE_DOUBLEACCOUNTING:
                    $dataBlockFont .= pack('C', 0x22);
                    $fontUnderline = 0;

                    break;
                case \Core\PhpSpreadsheet\Style\Font::UNDERLINE_SINGLE:
                    $dataBlockFont .= pack('C', 0x01);
                    $fontUnderline = 0;

                    break;
                case \Core\PhpSpreadsheet\Style\Font::UNDERLINE_SINGLEACCOUNTING:
