...
class Rels extends WriterPart
{
     ...
     * @return string XML Output
     */
    public function writeWorksheetRelationships(\Core\PhpSpreadsheet\Worksheet\Worksheet $pWorksheet, $pWorksheetId = 1, $includeCharts = false)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
        ...
        if (($pWorksheet->getDrawingCollection()->count() > 0) || (count($charts) > 0) || $drawingOriginalIds) {
            ...
            // (! synchronize with \Core\PhpSpreadsheet\Writer\Xlsx\Worksheet::writeDrawings)
    ...
    private function writeUnparsedRelationship(\Core\PhpSpreadsheet\Worksheet\Worksheet $pWorksheet, XMLWriter $objWriter, $relationship, $type): void
    {
        $unparsedLoadedData = $pWorksheet->getParent()->getUnparsedLoadedData();
     ...
     * @return string XML Output
     */
    public function writeDrawingRelationships(\Core\PhpSpreadsheet\Worksheet\Worksheet $pWorksheet, &$chartRef, $includeCharts = false)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
        ...
        while ($iterator->valid()) {
            if ($iterator->current() instanceof \Core\PhpSpreadsheet\Worksheet\Drawing
                || $iterator->current() instanceof MemoryDrawing) {
                // Write relationship for image drawing
                /** @var \Core\PhpSpreadsheet\Worksheet\Drawing $drawing */
     ...
     * @return string XML Output
     */
    public function writeHeaderFooterDrawingRelationships(\Core\PhpSpreadsheet\Worksheet\Worksheet $pWorksheet)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
    ...
    /**
     * @param $objWriter
     * @param \Core\PhpSpreadsheet\Worksheet\Drawing $drawing
     * @param $i
     *
     * @return int
     */
