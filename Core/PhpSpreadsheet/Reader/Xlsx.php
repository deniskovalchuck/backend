...
namespace Core\PhpSpreadsheet\Reader;

use Core\PhpSpreadsheet\Cell\Coordinate;
use Core\PhpSpreadsheet\Cell\Hyperlink;
use Core\PhpSpreadsheet\NamedRange;
use Core\PhpSpreadsheet\Reader\Security\XmlScanner;
use Core\PhpSpreadsheet\Reader\Xlsx\AutoFilter;
use Core\PhpSpreadsheet\Reader\Xlsx\Chart;
use Core\PhpSpreadsheet\Reader\Xlsx\ColumnAndRowAttributes;
use Core\PhpSpreadsheet\Reader\Xlsx\ConditionalStyles;
use Core\PhpSpreadsheet\Reader\Xlsx\DataValidations;
use Core\PhpSpreadsheet\Reader\Xlsx\Hyperlinks;
use Core\PhpSpreadsheet\Reader\Xlsx\PageSetup;
use Core\PhpSpreadsheet\Reader\Xlsx\Properties as PropertyReader;
use Core\PhpSpreadsheet\Reader\Xlsx\SheetViewOptions;
use Core\PhpSpreadsheet\Reader\Xlsx\SheetViews;
use Core\PhpSpreadsheet\Reader\Xlsx\Styles;
use Core\PhpSpreadsheet\ReferenceHelper;
use Core\PhpSpreadsheet\RichText\RichText;
use Core\PhpSpreadsheet\Settings;
use Core\PhpSpreadsheet\Shared\Date;
use Core\PhpSpreadsheet\Shared\Drawing;
use Core\PhpSpreadsheet\Shared\File;
use Core\PhpSpreadsheet\Shared\Font;
use Core\PhpSpreadsheet\Shared\StringHelper;
use Core\PhpSpreadsheet\Spreadsheet;
use Core\PhpSpreadsheet\Style\Border;
use Core\PhpSpreadsheet\Style\Borders;
use Core\PhpSpreadsheet\Style\Color;
use Core\PhpSpreadsheet\Style\NumberFormat;
use Core\PhpSpreadsheet\Style\Protection;
use Core\PhpSpreadsheet\Style\Style;
...
class Xlsx extends BaseReader
{
                                        ...
                                        if ($xmlDrawingChildren->oneCellAnchor) {
                                            foreach ($xmlDrawingChildren->oneCellAnchor as $oneCellAnchor) {
                                                if ($oneCellAnchor->pic->blipFill) {
                                                    ...
                                                    $objDrawing = new \Core\PhpSpreadsheet\Worksheet\Drawing();
    ...
    private static function readStyle(Style $docStyle, $style): void
    {
        ...
        if (isset($style->font)) {
            ...
            if (isset($style->font->u) && !isset($style->font->u['val'])) {
                $docStyle->getFont()->setUnderline(\Core\PhpSpreadsheet\Style\Font::UNDERLINE_SINGLE);
                ...
                foreach ($is->r as $run) {
                    if (!isset($run->rPr)) {
                        ...
                        if (isset($run->rPr->u) && !isset($run->rPr->u['val'])) {
                            $objText->getFont()->setUnderline(\Core\PhpSpreadsheet\Style\Font::UNDERLINE_SINGLE);
    ...
    /**
     * @param \Core\PhpSpreadsheet\Worksheet\Drawing $objDrawing
